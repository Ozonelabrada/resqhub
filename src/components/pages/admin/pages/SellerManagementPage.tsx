...
import { AlertCircle, CheckCircle, Users, FileText, Clock, Store, TrendingUp, Zap } from 'lucide-react';
...
export const SellerManagementPage: React.FC<{ hideHeader?: boolean }> = ({ hideHeader = false }) => {
  const [activeTab, setActiveTab] = useState<TabType>('management');
  const [searchQuery, setSearchQuery] = useState('');
  const [isActiveFilter, setIsActiveFilter] = useState<string | boolean>('');
  
  const [sellers, setSellers] = useState<UserRecord[]>([]);
    ...
    if (activeTab === 'management') {
      ...
      AdminService.getRidersWithCredits({
        ...
        .then((response) => {
          ...
          const transformedSellers: UserRecord[] = sellersData.map((seller: any) => ({
            id: seller.id?.toString() || seller.userId,
            name: seller.shopName || seller.storeName || `${seller.firstName} ${seller.lastName}`,
            email: seller.email,
            credits: seller.credits?.totalCreditsRemaining || 0,
            isActive: seller.isActive,
            isExempted: seller.isExempted,
            accessType: seller.subscriptionTier || 'free',
          }));
          
          setSellers(transformedSellers);
          setManagementPagination((prev: PaginationState) => ({
            ...prev,
            total: pagination.totalCount || sellersData.length,
          }));
          setLoading(false);
        })
        .catch((error) => {
          console.error('Failed to fetch sellers:', error);
          showNotification('error', 'Failed to load sellers');
          setLoading(false);
        });
    }
  ...
  useEffect(() => {
    if (activeTab === 'history') {
      setLoading(true);
      AdminService.getAllCreditHistory('seller')
        .then((response) => {
          const data = response.data || response;
  ...
  const handleTableToggleExemption = (row: UserRecord) => {
    ...
    setConfirmAction({
      ...
      id: row.id,
      message: `${row.isExempted ? 'Remove exemption' : 'Exempt'} ${row.name || 'this seller'}?`,
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (confirmAction.type === 'toggle-exemption') {
        // Get current seller to check if exemption state truly needs to change
        const currentSeller = sellers.find(s => s.id === confirmAction.id);
        if (currentSeller && currentSeller.isExempted !== confirmAction.exempted) {
          // Only toggle if state needs to change
          await AdminService.toggleSellerExemption(confirmAction.id);
        }
      }
  ...
  const sellerColumns: TemplateTableColumn[] = [
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-3 lg:grid-cols-4">
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:border-orange-500 focus:ring-orange-500"
          >
  ...
  const requestColumns: TemplateTableColumn[] = [
    ...
    { key: 'userId', label: 'Shop ID' },
    { key: 'userName', label: 'Shop Name' },
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="bg-gradient-to-r from-orange-50 to-red-50 p-8 rounded-xl border border-orange-100">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">Manage seller credit allocations and subscriptions</p>
              </div>
            </div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="p-6 bg-gradient-to-br from-orange-50 to-red-100 border-orange-200 hover:shadow-lg transition-shadow">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">Total Sellers</p>
                  <p className="text-2xl font-bold text-orange-600 mt-2">{managementPagination.total}</p>
                </div>
                <Users className="text-orange-300 opacity-50" size={32} />
              </div>
            </Card>
      ...
      <div className="flex gap-2 border-b-2 border-gray-200 bg-white rounded-t-lg shadow-sm">
        {['management', 'requests', 'history'].map((tab) => (
          <button
            ...
            className={cn('px-6 py-4 font-semibold flex items-center gap-2 transition-all border-b-4 -mb-0.5', activeTab === tab ? 'border-orange-600 text-orange-600 bg-orange-50' : 'border-transparent text-gray-600 hover:text-gray-900 hover:bg-gray-50')}
          >
            {tab === 'management' && <Users size={18} />}
            {tab === 'requests' && <FileText size={18} />}
            {tab === 'history' && <Clock size={18} />}
            <span>{tab === 'management' ? 'Seller Management' : tab === 'requests' ? 'Credit Requests' : 'History & Details'}</span>
          </button>
        ))}
      </div>
      ...
      {activeTab === 'management' && (
        <div className="space-y-6">
          <TemplateTable
            header={{ title: 'Seller Management', description: 'Grant, deduct, and manage seller credit allocations' }}
            columns={sellerColumns}
            data={sellers}
            actions={sellerActions}
            toolbar={sellerToolbar}
            loading={loading}
            empty={{ title: 'No sellers found', description: 'Try adjusting your filters or search terms' }}
      ...
      {activeTab === 'requests' && (
        <div className="space-y-6">
          <TemplateTable
            header={{ title: 'Seller Credit Requests', description: 'View and manage credit requests from sellers' }}
