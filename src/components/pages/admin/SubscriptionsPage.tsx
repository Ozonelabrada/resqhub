...
const SubscriptionsPage: React.FC = () => {
  ...
  const determineActiveTab = () => {
    ...
    if (pathname.includes('/subscriptions/riders')) {
      return 'riders';
    }
    if (pathname.includes('/subscriptions/sellers')) {
      return 'sellers';
    }
    if (pathname.includes('/subscriptions/events')) {
      return 'events';
    }
    if (pathname.includes('/subscriptions/service-providers')) {
      return 'service-providers';
    }
    // Default to communities
    return 'communities';
  };

  // Tab State
  const [activeTab, setActiveTab] = useState<'communities' | 'riders' | 'sellers' | 'events' | 'service-providers'>('communities');
  ...
  const handleSavePlan = async () => {
    try {
      ...
      // Update local state - Community plans only
